...
async function getMediaSource2(musicItem, quality) {
    if (Math.round(Math.random()) == 0) {
        let jx_url = await KongHen02(musicItem.songmid);
        if (!jx_url) {
        ...
        if (!jx_url) {
            jx_url = await KongHen02(musicItem.songmid);
        }
    }
    return jx_url;
}
...
// by 空痕音乐下载器
// https://www.123pan.com/s/x7ecVv-UYsAv.html
async function KongHen02(songmid) {
    try {
        let jx_item = (await (0, axios_1.default)({
            method: "GET",
            url: "https://api.khkj.xyz/music/qq.php?type=song&id=" + songmid,
            xsrfCookieName: "XSRF-TOKEN",
            withCredentials: true,
        })).data;
        return jx_item.data.purl;
    } catch (err) {
        return null;
    }
}
